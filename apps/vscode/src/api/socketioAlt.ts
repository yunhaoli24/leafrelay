...
import {
    ...
    type Identity,
    type ProjectEntity,
    type ProjectMessageResponseSchema,
...
export class SocketIOAlt {
    ...
    private get historyRefreshInterval(): number {
        return vscode.workspace.getConfiguration(EXTENSION_NAMESPACE).get<number>(keyHistoryRefreshInterval, 3) * 1000;//ms
    }

    private get messageRefreshInterval(): number {
        return vscode.workspace.getConfiguration(EXTENSION_NAMESPACE).get<number>(keyChatMessageRefreshInterval, 3) * 1000;//ms
    }

    private watchConfigurations() {
        return vscode.workspace.onDidChangeConfiguration((e) => {
                if (e.affectsConfiguration(`${EXTENSION_NAMESPACE}.${keyHistoryRefreshInterval}`)) {
                    this.vfsRefreshTask.interval = this.historyRefreshInterval;
                }

                if (e.affectsConfiguration(`${EXTENSION_NAMESPACE}.${keyChatMessageRefreshInterval}`)) {
    ...
    private async refreshVFS() {
        ...
        this.connectedUsers = this.connectedUsers.filter(user => {
            const DisconnectionTimeout = vscode.workspace.getConfiguration(EXTENSION_NAMESPACE).get<number>(keyInactiveTimeout, 180) * 1000;//ms
