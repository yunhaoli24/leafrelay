...
Changes made by other applications are detected and uploaded automatically, including while VS Code is in the background. A shared LeafRelay daemon owns the file-system watcher, so VS Code windows and `leafrelay serve` processes cannot upload the same change twice. Files matching the Local Replica ignore patterns are excluded. Synchronization diagnostics are available in the `LeafRelay` channel in the Output view and in `~/.leafrelay/daemon.log`.

After the first synchronization, `.overleaf/sync/` keeps a small project cursor, one JSON checkpoint per synchronized path, and content-addressed plain-text merge bases. Updating one file rewrites only that path record; it does not serialize the rest of the project again. Legacy `.overleaf/sync-state.json` data is not migrated. If the sharded state is absent, LeafRelay downloads the current Overleaf state and rebuilds checkpoints per path: identical and remote-only paths are established automatically, while divergent and local-only paths remain untouched and require an explicit choice. Subsequent project opens and reconnects reuse the latest history update response for both the current version and changed paths when possible, so unchanged file contents are not downloaded and an additional file-tree diff request is avoided. History requests are serialized and rate-limit responses honor `Retry-After`. Each successfully synchronized path persists its new fingerprint before the next path is processed, so a later conflict or failure does not discard completed progress. If synchronization ultimately fails, a deduplicated VS Code error notification links to the LeafRelay output log. Successful path logs include changed line ranges or binary byte counts and identify the local filesystem or Overleaf collaborator when that identity is available. Local symbolic links and paths below symbolic-link directories are excluded from upload, download, deletion, and sync state. A local replica stops watching and releases its Overleaf project lease as soon as its last attached client disconnects; other replicas with active clients continue independently.
